...
std::vector<std::unordered_set<int>> original_FM(
    const CSRMatrix &current_graph,
    ...
    bool speedup) 
{
    ...
    while (!FibHeap.empty()){
        ...
        if(total_gain>largest_gain+1e-8){
            ...
            max_improve_step = std::max(max_improve_step, move_node_num - largest_gain_step);
    ...
    if (move_history.empty()) {
        std::cout << "max_improve_step: " << max_improve_step << " node number:"<< n << std::endl;
        return CP;
    }
    ...
    for (int i = 1; i < move_history.size(); i++) {
    ...
    if (max_gain <= 1e-8) {
        ...
        for (size_t i = 0; i <= max_gain_index; ++i) {
            ...
            int dist = fpga_distances[original_fpga_id][new_fpga_id];
        ...
        std::cout << "max_improve_step: " << max_improve_step << " node number:"<< n << std::endl;
        for (auto const& [dist, count] : distance_counts) {
            std::cout << "distance:" << dist << " movement number: " << count << std::endl;
        }
        return CP;
    }
}
    ...
    const int allow_hop) 
{
    ...
    int k = fpga_distances.size();
    int r=2;
    double hyperedge_perference = 10.0;
    std::vector<int>nodes_fixed_in_fpga(k,0);
    ...
    for (int i = 0; i < hyperedges.size(); ++i) {
        ...
        if (CP[source].size() == 1) {
            int source_fpga = *CP[source].begin();
            hyperedge_fpga_count[i][source_fpga]++;
        }
        for (int drain : drains) {
            if (CP[drain].size() == 1) {
                int drain_fpga = *CP[drain].begin();
    ...
    for (int node_id = 0; node_id < n; ++node_id) {
        ...
        if (CP[node_id].size() == 1) {
            ...
            for (CSRMatrix::InnerIterator it(current_graph, node_id); it; ++it) {
                int neighbor_id = it.index();
                int neighbor_fpga_id = *CP[neighbor_id].begin();
                //priority[neighbor_fpga_id] += allow_hop*it.value(); //python wrong
                for (int fpga_id = 0; fpga_id < k; ++fpga_id) {
                    int dist = fpga_distances[neighbor_fpga_id][fpga_id];
    ...
    if (max_gain <= 1e-8) {
        ...
        for (size_t i = 0; i <= max_gain_index; ++i) {
            auto [node_id, original_fpga_id, new_fpga_id, total_gain] = move_history[i];
            //std::cout<<"node_id:"<<node_id<<" original_fpga_id:"<<original_fpga_id<<" new_fpga_id:"<<new_fpga_id<<std::endl;
            int dist = fpga_distances[original_fpga_id][new_fpga_id];
            distance_counts[dist]++;
            if (CP[node_id].size() == 1 && CP[node_id].count(original_fpga_id)) {
                CP[node_id] = {new_fpga_id};
...
int main(int argc, char* argv[]) {
    ...
    std::string netlist_file_path = "./ICCAD2021-TopoPart-Benchmarks/Titan23_Benchmarks/Titan23_Benchmarks/" + data; ///Generated_Benchmarks/Generated_Benchmarks/
    ...
    auto [netlist_matrix, init_fixed_nodes, init_hyperedges] = remake_netlist(netlist_matrix0, init_fixed_nodes0, init_hyperedges0);
    ...
    //int allow_hop = 2;
    auto uncoarsen_CP = initial_partitioning(current_graph, CP, fpga_distances, allow_hop_fpga_distances, current_node_weight, capacity, allow_hop);
    
    std::vector<int> assignment0(current_graph.rows());
    for (int j = 0; j < current_graph.rows(); ++j) {
        assignment0[j] = *uncoarsen_CP[j].begin();
    }
    
    print_partition_stats(assignment0, current_graph, fpga_distances, cur_hyperedges, cur_hyperedge_weight, allow_hop, "", true);
    ...
    //uncoarsen_CP = original_FM(current_graph, current_fixed_nodes, uncoarsen_CP, allow_hop_fpga_distances, current_node_weight, beta, capacity, speedup);
    uncoarsen_CP = FM(current_graph, current_fixed_nodes, uncoarsen_CP, fpga_distances, current_node_weight, beta, capacity, speedup, data, cur_hyperedges, cur_hyperedge_weight, allow_hop);
    ...
    for (int i=coarsen_level-1;i>=0;i--){
        ...
        if(0) {
            uncoarsen_CP = original_FM(graph_previous_level, fixed_node_previous_level, CP_previous_level, allow_hop_fpga_distances, node_weight_previous_level, beta, capacity, speedup);
        }
        else
            uncoarsen_CP = FM(graph_previous_level, fixed_node_previous_level, CP_previous_level, fpga_distances, node_weight_previous_level, beta, capacity, speedup, data, hyperedges_previous_level, hyperedge_weight_previous_level, allow_hop);
        
        std::vector<int> assignment2(nodenum);
        for (int j = 0; j < nodenum; ++j) {
            assignment2[j] = *uncoarsen_CP[j].begin();
        }
        //std::cout<<"after FM:"<<std::endl;
        //print_partition_stats(assignment2, graph_previous_level, fpga_distances, hyperedges_previous_level, hyperedge_weight_previous_level, allow_hop, "", true);
        ...
        if(0){
            std::vector<int> assignment(nodenum);
            for (int j = 0; j < nodenum; ++j) {
                assignment[j] = *uncoarsen_CP[j].begin();
            }
            print_partition_stats(assignment, graph_previous_level, fpga_distances, hyperedges_previous_level, hyperedge_weight_previous_level, allow_hop, "", true);
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        final_assignment[i] = *uncoarsen_CP[i].begin();
    }
    print_partition_stats(final_assignment, netlist_matrix, fpga_distances, init_hyperedges, init_hyperedge_weight, allow_hop, "Final ", true);
